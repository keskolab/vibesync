...
fn two_machine_roundtrip() {
    ...
    let report = sync::pull_dir(&CLAUDE_CODE, &b.home, ".claude", &b.tok, &mut b.state, &store, false, &|_| false, &store.list().unwrap(), &|| {}, &|_| {}).unwrap();
...
fn deleted_sessions_are_not_resurrected() {
    ...
    let report = sync::pull_dir(&CLAUDE_CODE, &a.home, ".claude", &a.tok, &mut a.state, &store, false, &|_| false, &store.list().unwrap(), &|| {}, &|_| {}).unwrap();
    assert_eq!(report.pulled, 0);
